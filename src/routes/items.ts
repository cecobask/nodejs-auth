...
export default (app) => {
    app.get('/item/', isAuth, attachCurrentUser, async (req, res, next) => {
        try {
    ...
    app.get('/item/:id', isAuth, attachCurrentUser, async (req, res, next) => {
        try {
            const user = req.currentUser;
    ...
    app.post('/item/', isAuth, attachCurrentUser, async (req, res, next) => {
        try {
            const user = req.currentUser;
    ...
    app.put('/item/:id', isAuth, attachCurrentUser, async (req, res, next) => {
        try {
            const user = req.currentUser;
            const itemDTO = req.body.item;
            const itemId = req.params.id;
    ...
    app.delete('/item/:id', isAuth, attachCurrentUser, async (req, res, next) => {
        try {
            const user = req.currentUser;
            const itemId = req.params.id;
            const itemServiceInstance = new ItemsService();
